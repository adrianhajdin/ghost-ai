...
- Input: project id, project name, current graph id, selected node ids, user message, recent Architect messages, and the sanitized CanvasDoc pyramid JSON loaded from `graph_root` through linked child layers.
...
- Input: sanitized CanvasDoc pyramid JSON containing the root graph, linked child graphs, nested layers, nodes, edges, labels, descriptions, metadata, custom/unknown architecture types, subcanvasRef values, and graph metadata.
- Execution: durable background task via the internal PostgreSQL-backed AI task runner.
