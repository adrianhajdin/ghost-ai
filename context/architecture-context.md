...
- `lib/prompt-pack` — LLM Prompt Pack v1 transport schema, Markdown/JSON export rendering, canvas improvement patch apply mechanics, and legacy/internal deterministic Prompt Pack compatibility from Design IR.
...
- Prompt Packs are generated by the LLM directly from sanitized CanvasDoc pyramid JSON. Prompt Packs are copy/download instruction artifacts only. Arc Forge does not execute Prompt Packs, build generated apps, or write back to repositories. The main Prompt Pack flow uses the AI task runner and provider abstraction; deterministic Design IR Prompt Pack compilation remains legacy/internal compatibility only.
...
- Optional canvas improvement proposals are previewed in the UI and applied only after explicit user approval. Apply is non-destructive in this version: update-node, add-node, add-edge, create-layer, and update-graph are allowed; destructive delete operations are reported as skipped issues.
...
- Deterministic Prompt Pack compilation from Design IR can remain for legacy/internal compatibility, but it is not the main user-facing Prompt Pack generator.
