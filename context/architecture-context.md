...
- Architect conversation messages are stored per project/graph in PostgreSQL, scoped by project access, sanitized before persistence, and kept separate from collaborator Chat.
...
- Persistence: user and assistant messages are stored per project/graph with role, content, createdAt, linked run id, and sanitized metadata. Recent messages are sent back to the LLM; collaborator Chat feed is not used for AI messages.
- Apply: canvas patches are never auto-applied. Supported operations are update-node, add-node, add-edge, create-layer, and update-graph. Unsupported destructive operations are skipped with explicit issues. Apply is authenticated, graph-safe, non-destructive, strips raw secrets/transient UI state, preserves custom/unknown types, writes CanvasDoc through the storage provider, and publishes a graph-scoped realtime snapshot for the active graph.
