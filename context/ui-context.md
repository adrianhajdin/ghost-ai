...
- Prompt Pack preview/export: the workspace navbar exposes a compact Prompt Pack action that opens a separate bounded cockpit-style modal. The main flow lets users choose Codex, Claude Code, or Generic AI Builder, choose full-project/current-layer/selected-node scope, add extra instructions, and generate an LLM-authored Prompt Pack from the sanitized canvas pyramid through the AI task runner. Preview tabs show Global Prompt, Layers, Nodes, Canvas Improvements, and JSON. Copy/download controls export the generated Markdown/JSON locally. Canvas improvements are previewed and applied only after explicit user approval. Arc Forge does not execute Prompt Packs, build the app, or write external repositories. Nimbus is not included as a Prompt Pack target in this version.
- AI Architect draft preview: the AI sidebar includes an Architect tab where users describe the desired system or current layer, choose draft complexity, generate a structured proposal, review safety/apply issues, inspect proposed graphs/layers, nodes, and relations, and explicitly apply or clear the proposal. Apply/Clear actions stay visible near the preview status area and also at the preview footer. The latest generated draft is restored from the persisted AI run when the sidebar remounts until the user applies, clears, or the run fails. The copy states that AI proposes the architecture, Arc Forge previews/applies/saves/protects the canvas, and layers work like a pyramid.
