...
- Canvas graph breadcrumb: a small glass overlay sits below the navbar; it identifies the current graph and, inside service drill-down canvases, exposes an explicit `Back to System canvas` control so users can return to the root without relying on browser Back. It must avoid overlapping the sidebars or inspector.
...
- Prompt Pack preview/export: the workspace navbar exposes a compact Prompt Pack action that opens a separate bounded cockpit-style modal. Prompt Packs are generated from Design IR. Prompt Packs are copy/download instruction artifacts only. Arc Forge does not execute Prompt Packs. Supported selector options are Codex, Claude Code, and Generic AI Builder. Nimbus is not included as a Prompt Pack target in this version.
- AI Architect draft preview: the AI sidebar includes an Architect tab where users describe the desired system, choose draft complexity, generate a structured proposal, review validation issues, inspect grouped semantic nodes and relations, and explicitly apply or clear the proposal. Apply/Clear actions stay visible near the preview status area and also at the preview footer. The latest generated draft is restored from the persisted AI run when the sidebar remounts until the user applies, clears, or the run fails. The copy states that AI proposes an architecture draft, the user stays in control, and Arc Forge does not build or execute the app.
