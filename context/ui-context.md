...
- Add Semantic Node exposes the compact Canvas v2 taxonomy by default and keeps advanced/common-but-less-frequent types behind a More affordance. Phase 3 contextual advanced types — Reference Proxy, Runtime / Deployment Unit, Observability / Control Plane, and AI Component — stay behind More or Architect suggestions rather than permanent default toolbar icons. Child layers additionally expose internal detail templates such as endpoints, entities, event contracts, rules, and policies.
- Connect Relationship exposes quick relationship types first, then advanced relationships, and newly drawn edges persist both `relationshipType` and compatible label data. The relationship picker includes compact guidance: use calls for request/response, reads/writes for state, and publishes/consumes for async events.
- Nodes show a compact semantic type badge; unclassified nodes use warning styling. Boundary/exposure and reference proxy indicators should stay subtle chips/outlines, not large overlays.
...
- The inspector uses progressive disclosure instead of one long form. Node metadata is grouped into Overview, Interfaces, Data, Events, Security, Operations, optional Reference Proxy, Prompt Pack Notes, Open Questions, and Child Layer. Trust/boundary metadata lives under Overview/Security/Data, runtime/observability/AI metadata lives in Operations/Security, and proxy fields are shown only for reference-proxy or nodes with reference metadata. Edge metadata is grouped into Overview, Mechanism, Reliability, Data / Events, Security / Trust, and Notes.
...
- Edge relationship type is edited from the inspector and may show a compact badge only during active edge interaction. Low-risk edge affordances can vary line styles by relationship type: reads dashed/subtle, writes solid, publishes/consumes message-like dashed, monitors/runs_on dotted/subtle, and authenticates_via with a security accent.
- Semantic Scan appears as a compact expandable cockpit chip when nothing is selected, not as a large warning card. Findings are grouped by category, advisory findings can be snoozed or marked intentional per graph, and blocking safety findings remain visible. Each finding exposes compact Ask/Fix actions that open Architect and send the finding context directly into the project-wide LLM thread for explanation or a user-approved patch proposal. Semantic warnings are advisory and do not block saving, Prompt Pack generation, Architect proposals, or child-layer creation.
...
- Prompt Pack preview/export: the workspace navbar exposes a compact Prompt Pack action that opens a separate bounded cockpit-style modal. The flow lets users choose Codex, Claude Code, or Generic AI Builder, choose full-project/current-layer/selected-node scope, add extra instructions, and generate an LLM-authored Prompt Pack from the sanitized canvas pyramid and LLM Context Pyramid v2 through the AI task runner. There is no non-LLM Prompt Pack generator, route, or fallback. Preview tabs show Global Prompt, Layers, Nodes, Canvas Improvements, and JSON. Copy/download controls export the generated Markdown/JSON locally. Canvas improvements are previewed and applied only after explicit user approval. Preview copy should show server validation, affected graph IDs, and blocking issues when present. Arc Forge does not execute Prompt Packs, build the app, or write external repositories. Nimbus is not included as a Prompt Pack target in this version.
- AI Workspace sidebar: the sidebar exposes exactly one LLM interaction tab, `Architect`, plus `Chat` for human collaborator discussion and `Specs` for spec generation/browsing. Architect is a conversational cockpit workspace: header/context strip, persisted project-wide message thread with graph provenance, user input, quick actions, canvas-aware proposal preview/apply, and a Prompt Pack entry point. It reads the sanitized canvas pyramid and LLM Context Pyramid v2 through the task runner and may propose non-destructive canvas patches for any graph in the architecture pyramid, but the canvas changes only after the user clicks apply. Patch previews show affected graph IDs, operation status, blocking issues, and temporary-to-durable ID mappings so users understand exactly what will happen before apply. Chat must remain project chat for workspace members, not an AI assistant surface. Specs remains dedicated to spec generation, preview, and download. No Design AI tab is exposed.
