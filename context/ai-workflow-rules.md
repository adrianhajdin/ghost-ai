...
- Do not add deterministic architecture evaluators, deterministic Prompt Pack authoring, child-layer semantic-type permission gates, app execution, repo write-back, unattended patch application, image/multimodal input, or extra AI editing personas unless a future product contract explicitly changes that scope.
...
- Keep graph summary caches and LLM Context Pyramid payloads derived from CanvasDoc. They may focus the current graph, selected nodes, related layers, semantic findings, and conversation provenance, but they must not persist transient UI state, raw secrets, or become an alternate source of truth.
- LLM canvas patch proposals may use temporary IDs for newly proposed nodes and edges. Preview/apply must resolve temporary IDs to durable IDs before persistence, reject unknown existing IDs or unknown temporary references, and keep apply user-approved.
