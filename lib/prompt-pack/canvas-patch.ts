...
import {
  extractCanvasImprovementProposal,
  sanitizePromptPackPatchRecord,
...
function nodeFromPatch(
  ...
  const metadata = sanitizePromptPackPatchRecord(operationNode.metadata)
...
function edgeFromPatch(
  ...
  const metadata = sanitizePromptPackPatchRecord(operationEdge.metadata)
  const semanticTypeText =
...
function applyNodePatchData(node: CanvasNode, patch: Record<string, unknown>) {
  const sanitizedPatch = sanitizePromptPackPatchRecord(patch)
...
export async function applyLlmCanvasImprovementProposal(input: {
  ...
  const proposal: LlmCanvasImprovementProposal = extractCanvasImprovementProposal(
    input.proposal
  )
  ...
  for (const [operationIndex, operation] of proposal.operations.entries()) {
    ...
    if (operation.op === "update-graph") {
      ...
      const patch = sanitizePromptPackPatchRecord(updateGraphOperation.patch)
      const nextDoc: CanvasDocV1 = {
        ...
        panels: patch.metadata
          ? { ...doc.panels, llmPromptPackMetadata: patch.metadata }
          : doc.panels,
      }
      docsByGraphId.set(graphId, nextDoc)
      dirtyGraphIds.add(graphId)
